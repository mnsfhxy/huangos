...
/* 来自PS/2键盘的中断 */
{
	...
	boxfill(binfo->vram, binfo->screen_x, COL8_000000, 0, 0, 32 * 8 - 1, 15);
	print_string(binfo->vram, binfo->screen_x, 0, 0, COL8_FFFFFF, "INT 21 (IRQ-1) : PS/2 keyboard");
	for (;;) {
		io_hlt();
	}
}
...
/* 来自PS/2鼠标的中断 */
{
	BOOTINFO *binfo = ( BOOTINFO *) ADR_BOOTINFO;
	boxfill(binfo->vram, binfo->screen_x, COL8_000000, 0, 0, 32 * 8 - 1, 15);
	print_string(binfo->vram, binfo->screen_x, 0, 0, COL8_FFFFFF, "INT 2C (IRQ-12) : PS/2 mouse");
	for (;;) {
		io_hlt();
	}
}
